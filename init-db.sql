...
-- ==================================================
-- 1. Classes Table  (must be before users due to FK)
-- ==================================================
...
-- NOTE: user_id and book_id are nullable
--   user_id  -> NULL when event is from an anonymous (not logged-in) user
--   book_id  -> NULL when the book cannot be resolved from the request
-- ==================================================
CREATE TABLE IF NOT EXISTS tracking_events (
    ...
    CONSTRAINT fk_tracking_events_book FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE
);

CREATE INDEX idx_tracking_events_user      ON tracking_events(user_id);
CREATE INDEX idx_tracking_events_book      ON tracking_events(book_id);
...
-- NOTE: book_id is nullable
--   heartbeat only tracks reading time; it does not carry a book_id
-- ==================================================
CREATE TABLE IF NOT EXISTS active_sessions (
    id UUID PRIMARY KEY,               -- client-generated session UUID (sent in heartbeat)
    ...
    CONSTRAINT fk_active_sessions_book FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE
);

CREATE INDEX idx_active_sessions_user         ON active_sessions(user_id);
CREATE INDEX idx_active_sessions_book         ON active_sessions(book_id);
...
CREATE INDEX idx_active_sessions_meta_tags    ON active_sessions USING GIN (meta_tags);

-- ==================================================
-- 7. User Progress Table  (book-level: last chapter read)
-- ==================================================
CREATE TABLE IF NOT EXISTS user_progress (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL,
    book_id UUID NOT NULL,
    last_chapter_id VARCHAR(255) NOT NULL,
    last_accessed_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT user_progress_user_book_key UNIQUE (user_id, book_id),
    CONSTRAINT user_progress_user_fkey FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT user_progress_book_fkey FOREIGN KEY (book_id) REFERENCES books(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS chat_turns (
    ...
    created_time TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT chat_turns_session_turn_unique UNIQUE (session_id, turn_no),
    CONSTRAINT fk_chat_turns_session FOREIGN KEY (session_id) REFERENCES chat_sessions(id) ON DELETE CASCADE
);

CREATE INDEX idx_chat_turns_session ON chat_turns(session_id);
CREATE INDEX idx_chat_turns_created ON chat_turns(created_time);
...
-- Books
-- NOTE: Song of Roland uses 44444444-... (dddddddd-... is reserved for the test student user)
INSERT INTO books (id, title, author) VALUES
    ...
    ('44444444-4444-4444-4444-444444444444', 'The Song of Roland',  'Anonymous')
ON CONFLICT (id) DO NOTHING;

-- Class-Book Access
--   Class 11 (Grade 10): Oedipus, Odyssey, Song of Roland
--   Class 22 (Grade 11): Iliad, Song of Roland
...
-- Users  (password: test12345 for all)
